<script>
	...
	const CartHandler = () => {
		...
		// postAddCart(productInCart); not work in server
	};
</script>

{#if fetchedProducts.length > 0}
	<div class="mt-10 flex items-center justify-between">
		<p class="font-semibold">Your Bag</p>
		<button class="rounded-sm bg-zard px-8 py-2" on:click={CartHandler}>Order</button>
	</div>
			...
			<div class="w-full border-b-2 border-zard py-2">
				...
				<div class="mt-4 flex items-center justify-between">
					...
					<button
						...
						on:click={() => removeCartItem(item.id)}>Delete</button
					>
				</div>
			</div>
		{/each}
...
{:else}
	<p class="h-min-screen mt-20 flex w-full flex-col items-center justify-center text-center">
		Your cart is empty!
	</p>
{/if}
