<script>
	...
	async function fetchCartItems() {
		try {
			// Access the current cart items using $productInCart
	...
	// Remove an item from both the cart and the fetched products
	function removeCartItem(id) {
		// Remove from the store using `update` to ensure reactivity
		productInCart.update((cart) => cart.filter((item) => item.id !== id));

		// Remove from the fetched products array
