<script>
	...
	function handleChange(e) {
		...
		fetchSearchProduct({ search: e.target.value });
	}
	console.log('output of search', $searchProduct);
</script>
...
{:else if $search.length !== 0 || $Products.length !== 0}
	<div>
		<div class="div flex items-center justify-between">
			...
			<input bind:value={$search} placeholder="Search" on:input={handleChange} />
		</div>

		<div class="my-10 grid grid-cols-3 gap-10">
			{#each $searchProduct ? $searchProduct : $Products as item}
