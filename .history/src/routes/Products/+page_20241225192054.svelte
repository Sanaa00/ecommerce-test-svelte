<script>
	...
	const addCartHandler = (id) => {
		...
		const existingProduct = $productInCart.find((item) => item.id === id);
		...
		if (existingProduct) {
			...
			console.log('Product quantity updated:', $productInCart);
		} else {
			// Add new product to the cart
			productInCart.set([...$productInCart, { id: id, quantity: 1 }]);
