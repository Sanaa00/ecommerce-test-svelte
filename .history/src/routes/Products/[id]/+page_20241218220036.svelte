<script>
	...
	// Reactively fetch products by category if $singleProducts.category exists
	$: if ($singleProducts?.category) {
		fetchProductByCategory($singleProducts.category);
	}
	{
		console.log('by category', $productByCategory);
	}
</script>
	...
	<div class="flex justify-between">
		<div class="mt-10 w-full">
			...
			<div class="mt-5 flex items-center justify-between">
				<p>Brand: {$singleProducts.brand}</p>
				<p>Category: {$singleProducts.category}</p>
			</div>
